...
This project contains the **7.0.0.0** version of the **Kinetica Spark Connector**
...
The connector JAR can be built with *Maven* as follows:
    ...
    $ git clone https://github.com/kineticadb/kinetica-connector-spark.git -b release/v7.0.0 --single-branch
...
``target`` directory:
    ...
    target/kinetica-spark-7.0.0.0-SNAPSHOT-jar-with-dependencies.jar
...
referenced later in this guide for use in testing the *Spark* connector:
    ...
    target/kinetica-spark-7.0.0.0-SNAPSHOT-tests.jar
...
``<SparkMasterPort>``, & ``<PropertiesFile>``:
    ...
    $ spark-submit \
       ...
       kinetica-spark-7.0.0.0-SNAPSHOT-jar-with-dependencies.jar <PropertiesFile>


To launch the *Ingest/Egress Processor* or *Streaming Processor* through the
*Spark* shell, run:

    $ spark-shell --jars kinetica-spark-7.0.0.0-SNAPSHOT-jar-with-dependencies.jar


To run the *Ingest/Egress Processor* through the *PySpark* shell:

    $ pyspark --jars kinetica-spark-7.0.0.0-SNAPSHOT-jar-with-dependencies.jar
...
``scripts/loader`` directory, as shown below.
    ...
    + spark-submit --class com.kinetica.spark.SparkKineticaDriver
        ...
        --driver-java-options -Dlog4j.configuration=file:/opt/spark-test/kinetica-spark-7.0.0.0/scripts/loader/log4j.properties
        ../../target/kinetica-spark-7.0.0.0-SNAPSHOT-jar-with-dependencies.jar csv-test.properties
    [...]
...
(``kinetica-spark-7.0.0.0-SNAPSHOT-jar-with-dependencies.jar``) have been copied to the
...
```shell
$ spark-shell --jars /opt/gpudb/connectors/spark/kinetica-spark-7.0.0.0-SNAPSHOT-jar-with-dependencies.jar
```
...
The test JAR, ``kinetica-spark-7.0.0.0-SNAPSHOT-tests.jar``, created in the
...
$ spark-submit \
    ...
    --jars /opt/gpudb/connectors/spark/kinetica-spark-7.0.0.0-SNAPSHOT-tests.jar \
       /opt/gpudb/connectors/spark/kinetica-spark-7.0.0.0-SNAPSHOT-jar-with-dependencies.jar \
...
```shell
$ pyspark --jars /opt/gpudb/connectors/spark/kinetica-spark-7.0.0.0-SNAPSHOT-jar-with-dependencies.jar
```
...
options = {
    "database.url" : url,
    "database.jdbc_url" : "jdbc:kinetica://%s:9191;URL=%s" % (host, url),
    "database.username" : username,
    "database.password" : password,
    "table.name" : "airline",
    "table.is_replicated" : "false",
...
$ spark-submit \
    --master "spark://<SparkMasterHostName/IP>:<SparkMasterPort>" \
    --jars /opt/gpudb/connectors/spark/kinetica-spark-7.0.0.0-SNAPSHOT-jar-with-dependencies.jar \
...
``/opt/gpudb/connectors/odbcserver/client/lib/com.kinetica.jdbc.core.jdbc42.Driver.jar``) to be
...
```shell
$ spark-shell --jars \
    /opt/gpudb/connectors/spark/kinetica-spark-7.0.0.0-jar-with-dependencies.jar, \
    /opt/gpudb/connectors/odbcserver/client/lib/com.kinetica.jdbc.core.jdbc42.Driver.jar
```
...
val url = s"http://${host}:9191"
val options = Map(
   "url" -> s"jdbc:kinetica://${host}:9191;URL=${url}",
   "driver" -> "com.kinetica.jdbc.core.jdbc42.Driver",
   "database.username" -> username,
   "database.password" -> password,
   "dbtable" -> s"""(
      SELECT
...
at ``/opt/gpudb/connectors/odbcserver/client/lib/com.kinetica.jdbc.core.jdbc42.Driver``) to be
...
$ spark-shell --jars \
    /opt/gpudb/connectors/spark/kinetica-spark-7.0.0.0-SNAPSHOT-jar-with-dependencies.jar, \
    /opt/gpudb/connectors/odbcserver/client/lib/com.kinetica.jdbc.core.jdbc42.Driver
```
...
val password = "<Password>"
val url = s"http://${host}:9191"
val jdbcUrl = s"jdbc:kinetica://${host}:9191;URL=${url}"
val options = Map(
   "database.url" -> url,
