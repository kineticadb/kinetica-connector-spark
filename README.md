...
This example assumes the ``2008.csv`` and *Spark* connector JAR
(``kinetica-spark-6.2.<X>.<Y>-jar-with-dependencies.jar``) have been copied to
...
| ``egress.offset``                  | ``0``       | The offset from which point in the table records should be extracted.  If the offset is beyong the table size, an empty dataframe will be returned.  Must be a postivie integer.  **Egress Processor Only**
...
| Property Name                    | Default   | Description
| :---                             | :---      | :---
| ``ssl.bypass_cert_check``        | ``false`` | Deprecated option since version 6.2.3.0; internally ignored.  Instead, if ``ssl.truststore_jks`` is provided, then the server certificate will be verified; if not provided, the check will be skipped.
